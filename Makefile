SCHEME := $(shell sed -n 's/scheme "\(.*\)"/\1/p' Scanfile)
PROJECT := $(shell sed -n 's/project "\(.*\)"/\1/p' Scanfile)

.PHONY: check_tools

default: check_tools
	SCAN_OUTPUT_TYPES="" scan --clean --destination "platform=iOS Simulator,name=iPhone 5s" --code_coverage
	slather coverage --verbose --output-directory "${TMPDIR}/${SCHEME} Coverage Report" --html --show --ignore "../**/Contents/Developer/**" --scheme "${SCHEME}" "${PROJECT}" || true
