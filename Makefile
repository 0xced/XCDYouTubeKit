...
test_macOS_report: check_scan
	scan                           --output_directory "${CIRCLE_TEST_REPORTS}/junit/" --output_types junit --custom_report_file_name test-results-macOS.xml

test_iOS_report: check_scan
	scan --device "iPhone 5s"      --output_directory "${CIRCLE_TEST_REPORTS}/junit/" --output_types junit --custom_report_file_name test-results-iOS.xml --code_coverage --xcargs "OBJROOT=build GCC_GENERATE_TEST_COVERAGE_FILES=YES GCC_INSTRUMENT_PROGRAM_FLOW_ARCS=YES"

test_tvOS_report: check_scan
	scan --device "Apple TV 1080p" --output_directory "${CIRCLE_TEST_REPORTS}/junit/" --output_types junit --custom_report_file_name test-results-tvOS.xml
