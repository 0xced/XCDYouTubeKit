...
NSString *const XCDYouTubeVideoOptionsAlternativeStreamFlag = @"XCDYouTubeVideoOptionsAlternativeStreamFlag";
...
- (instancetype) initWithIdentifier:(NSString *)identifier info:(NSDictionary *)info playerScript:(XCDYouTubePlayerScript *)playerScript response:(NSURLResponse *)response error:(NSError * __autoreleasing *)error
{
	...
	BOOL shouldUseAlternativeStream = [(NSNumber *)info[XCDYouTubeVideoOptionsAlternativeStreamFlag] boolValue];
	NSDictionary *alternativeStreamData = XCDStreamingDataWithString(info[@"player_response"]);
	NSArray <NSDictionary *> *alternativeFormats = alternativeStreamData[@"formats"];
	NSArray <NSDictionary *> *alternativeAdaptiveFormats = alternativeStreamData[@"adaptiveFormats"];
	
	NSMutableDictionary *userInfo = response.URL ? [@{ NSURLErrorKey: (id)response.URL } mutableCopy] : [NSMutableDictionary new];
	
	if (streamMap.length > 0 || httpLiveStream.length > 0 || shouldUseAlternativeStream || alternativeFormats.count > 0 || alternativeAdaptiveFormats.count > 0)
	{
		...
		{
			NSMutableArray <NSDictionary *> *allAlternativeFormats = [alternativeFormats mutableCopy];
			[allAlternativeFormats addObjectsFromArray:alternativeAdaptiveFormats];
			
			for (NSDictionary *stream in allAlternativeFormats)
			{
				NSString *urlString = stream[@"url"];
				NSString *itag = stream[@"itag"];
