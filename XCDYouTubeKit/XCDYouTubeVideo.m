...
- (instancetype) initWithIdentifier:(NSString *)identifier info:(NSDictionary *)info playerScript:(XCDYouTubePlayerScript *)playerScript response:(NSURLResponse *)response error:(NSError * __autoreleasing *)error
{
	...
	if (streamMap.length > 0 || httpLiveStream.length > 0 || captionsMap.length > 0)
	{
		...
		for (NSString *captionQuery in captionQueries) {
			...
			NSString *captionURLString = captionStream[@"u"];
			NSString *captionVersion = captionStream[@"v"];
			if (captionURLString && languageCode)
				
			{
				if([captionVersion hasPrefix:@"a"]) {
					//Indicates the caption was auto generated
					autoGeneratedCaptionURLs[languageCode] = [NSURL URLWithString:captionURLString];
				} else {
				
					captionURLs[languageCode] = [NSURL URLWithString:captionURLString];
